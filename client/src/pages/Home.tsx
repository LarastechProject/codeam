...
export default function Home() {
  ...
  const createAssignment = trpc.assignment.create.useMutation({
    onSuccess: async () => { toast.success("Assignment created"); setAssignmentTitle(""); setAssignmentInstructions(""); await assignmentList.refetch(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  if (!isAuthenticated) return (
    <div className="min-h-screen bg-[#071A52] text-white grid place-items-center p-6">
      <div className="w-full max-w-2xl border border-cyan-300/40 bg-[#0B2770] p-10 shadow-2xl">
        ...
        <Button className="mt-8 bg-cyan-300 text-[#071A52] hover:bg-cyan-200" onClick={() => startLogin()}>Sign in to continue</Button>
      </div>
    </div>
  );
